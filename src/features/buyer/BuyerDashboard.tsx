import { CloudSun, ShoppingBag, PackageSearch } from 'lucide-react'
import { mockRoutes } from '../routes/routeMockData'
import { getBuyerRoutes } from '../routes/roleRouteData'
import RouteMap from '../routes/RouteMap'
import BottomNav from '../../components/BottomNav'

export default function BuyerDashboard() {
  const routes = getBuyerRoutes(mockRoutes)
    ...
    <div className="min-h-dvh bg-[#050b06] text-[#e8f5e9] flex flex-col pb-20">
      <div className="flex-1 overflow-y-auto px-4 pt-6 pb-4 space-y-5">
        <header className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-bold text-[#e8f5e9]">Hola, María</h1>
            <p className="text-xs text-[#80cbc4]">Compradora</p>
          </div>
        ...
        <section>
          ...
          <div className="space-y-2">
            {cosechas.map((c) => (
              <div
                key={c.id}
                className="flex items-center gap-3 bg-[#0c1a0e] border border-[rgba(0,230,118,0.06)] rounded-xl px-4 py-3"
              >
                <div className="w-10 h-10 rounded-lg bg-[rgba(0,230,118,0.08)] flex items-center justify-center text-lg shrink-0">
                  <ShoppingBag size={18} className="text-[#00e676]" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold truncate">{c.producto}</p>
                  <p className="text-xs text-[#80cbc4]">{c.cantidad} · {c.ubicacion}</p>
                </div>
          ...
          <button
            type="button"
            className="w-full mt-3 py-3 rounded-xl border border-dashed border-[rgba(0,230,118,0.15)] text-sm font-semibold text-[#80cbc4] hover:bg-[rgba(0,230,118,0.03)] transition-colors flex items-center justify-center gap-2"
          >
